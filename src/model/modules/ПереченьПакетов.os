...
Процедура Инициализировать(Знач Путь) Экспорт
	...
	Чтение.Закрыть();
	База = Парсер.ПрочитатьJSON(Текст, , , Истина);
...
Функция ПолучитьПакеты() Экспорт
	...
	ТаблицаПакетов.Колонки.Добавить("dependencies");
	...
	Для Каждого КлючЗначениеПакет Из База Цикл
		...
		Если Объект.Свойство("dependencies") Тогда
			СтрТаблицы.dependencies = Объект["dependencies"];
		Иначе
			СтрТаблицы.dependencies = Новый Массив;	
		КонецЕсли;
	КонецЦикла;
