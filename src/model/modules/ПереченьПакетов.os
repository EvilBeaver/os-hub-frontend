...
Процедура ЗагрузитьБазуПакетовИзБД()
	
	Отбор = Новый Соответствие();
	Отбор.Вставить("Имя", "stable");
	Канал = МенеджерБазыДанных.МенеджерСущностей.ПолучитьОдно(Тип("Канал"), Отбор);
	
	РезультатПоиска = МенеджерБазыДанных.МенеджерСущностей.Получить(Тип("Пакет")); // сортировка?
	Для Каждого Результат Из РезультатПоиска Цикл
		
		НоваяСтрока = ТаблицаПакетов.Добавить();
		НоваяСтрока.Название = Результат.Наименование;
		НоваяСтрока.Описание = Результат.Описание;
		НоваяСтрока.ПутьХранения = ОбъединитьПути(КаталогХранения, Канал.Каталог, Результат.Код);
		// НоваяСтрока.Автор = Результат.Автор; 
		НоваяСтрока.Версии = Новый Массив;
		НоваяСтрока.АдресРепозитория = Результат.СсылкаНаПроект;
		
		// запрос в цикле
		Отбор = Новый Соответствие();
		Отбор.Вставить("Канал", Канал.Код);
		Отбор.Вставить("Пакет", Результат.Код);
		ПакетКанала = МенеджерБазыДанных.МенеджерСущностей.ПолучитьОдно(Тип("ПакетКанала"), Отбор);
		Если ПакетКанала <> Неопределено Тогда
			НоваяСтрока.АктуальнаяВерсия = ПакетКанала.АктуальнаяВерсия;
		Иначе
			НоваяСтрока.АктуальнаяВерсия = "";
		КонецЕсли;
...
Функция ДанныеПакетаПоКоду(Код) Экспорт
	...
	// Сообщить("Пакет канала: " + ПакетКанала.Код, СтатусСообщения.ОченьВажное);
	...
	Данные.ПутьХранения = ОбъединитьПути(КаталогХранения, Канал.Каталог, Пакет.Код);
	
	КоллекцияВерсий = Новый Массив; // НомерВерсии, ДатаВерсии
