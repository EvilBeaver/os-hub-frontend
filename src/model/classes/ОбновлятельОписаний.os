...
Процедура Инициализация(Знач КаталогСкачивания) Экспорт
	...
	Канал = ГлобальныйКонтекст.КаталогПакетов;
...
Процедура СохранитьФайлОписанияДляПорции(СписокРепозиториев)
	...
	Для Каждого Репозиторий Из СписокРепозиториев Цикл
		...
		Если Пакет = Неопределено Тогда
			Отбор = Новый Соответствие();
			Отбор.Вставить("ИмяПроекта", ИмяПакета);
			Пакет = МенеджерБазыДанных.МенеджерСущностей.ПолучитьОдно(Тип("Пакет"), Отбор);
		КонецЕсли;

		Если Пакет = Неопределено Тогда
			Лог.Информация("Пропускаю репо %1, т.к. не найден пакет в базе данных", Репозиторий.Имя);
			Продолжить;	
		КонецЕсли;
		...
		Если Не ЗначениеЗаполнено(Пакет.ПутьКОписанию) Тогда
			Пакет.ПутьКОписанию = ОбъединитьПути(Канал, ИмяПакета, Контент.Имя); // относительный путь
