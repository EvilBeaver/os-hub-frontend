...
Процедура ПрочитатьКартуИмен()
	ФайлКарты = ОбъединитьПути(ПереченьПакетов.КаталогХраненияПакетов(),"nameRemap.json");
...
Процедура ОбновитьКешПакетов() Экспорт
	...
	Для Каждого ДанныеРепозитория Из СписокРепозиториев Цикл
		Лог.Информация("Получаю URL для " + ДанныеРепозитория["name"]);
	...
	Для Каждого СтрокаОписания Из Таблица Цикл
		Лог.Информация("Запись кеша для %1", СтрокаОписания.Название);
