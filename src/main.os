...
Процедура ПриНачалеРаботыСистемы()
	...
	Если ПолучитьПеременнуюСреды("OSHUB_IMPORT") <> Неопределено Тогда
		Если Не МиграцияДанных.ЗапуститьИмпортВБД() Тогда
			ВызватьИсключение("Не удалось загрузить данные в БП. Запуск hub невозможна.");
		КонецЕсли;
	КонецЕсли;
