...
Функция Push() Экспорт
	...
	Попытка
		Пакет = ОбщегоНазначения.ЗафиксироватьПакет(КаталогПубликации, ДанныеФайла, ИмяФайла, ОписаниеПакета, ЭтоТестоваяСборка);
	Исключение
		Возврат ВернутьОтвет(
			ОбщегоНазначения.ВычислитьКодОшибки(ИнформацияОбОшибке()), 
	...
	Если ГлобальныйКонтекст.ОтправлятьУведомлениеВТелеграмм Тогда
		УправлениеУведомлениями.УведомитьОРелизе(
