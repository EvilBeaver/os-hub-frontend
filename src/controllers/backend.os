...
Функция Push() Экспорт
	...
	Попытка		
		...
		Пользователь = ОбщегоНазначения.ПользовательGitHubПоТокену();
	...
	Попытка
		ОбщегоНазначения.ЗафиксироватьПакет(КаталогПубликации, ДанныеФайла, ИмяФайла, ОписаниеПакета, ЭтоРелизныйКанал(Канал));
	Исключение
		Возврат ВернутьОтвет(
			ОбщегоНазначения.ВычислитьКодОшибки(ИнформацияОбОшибке()), 
